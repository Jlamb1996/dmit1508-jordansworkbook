...
-- we express relationships between tables in our design through FOREIGN KEY constraints. But those constraints simply check/restrict information that is stored in the foreign key column. It doesnt actually physically connect the tables. all of the tables are independent. That means when we try to query information from multiple related tables, we have to state the connection between the tables. That is, we have to state how they join together.
